  ...
  if (node != ASN1_TYPE_EMPTY)
    {
      ...
      if (res != ASN1_SUCCESS)
	{
	  shishi_error_printf (handle, "Could not DER encode KDC-REQ: %s\n",
			       errorDescription);
	  return SHISHI_ASN1_ERROR;
	}
			 ...
			 ASN1_TYPE apreq, ASN1_TYPE * ticket)
{
  ...
  *ticket = ASN1_TYPE_EMPTY;
  ...
  buflen = BUFSIZ;
  res =
    asn1_read_value (apreq, "AP-REQ.ticket.enc-part.etype", buf, &buflen);
  if (res != ASN1_SUCCESS)
    goto error;
  ...
  buflen = BUFSIZ;
  res =
    asn1_read_value (apreq, "AP-REQ.ticket.enc-part.kvno", buf, &buflen);
  if (res != ASN1_SUCCESS)
    goto error;
